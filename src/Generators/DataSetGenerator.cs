    ...
    : IAutoGenerator
{
    ...
    class UntypedDataSetGenerator : DataSetGenerator
    {
        public override object Generate(AutoGenerateContext context)
        {
            ...
            if (!DataTableGenerator.TryCreateGenerator(typeof(DataTable), out DataTableGenerator? tableGenerator))
        ...
        public override object Generate(AutoGenerateContext context)
        {
            ...
            while (allTables.Count > 0)
            {
                ...
                for (int i = 0; i < allTables.Count; i++)
                {
                    ...
                    if (!DataTableGenerator.TryCreateGenerator(table.GetType(), out DataTableGenerator? tableGenerator))
